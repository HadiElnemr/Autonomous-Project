...
def callback_Act_St(data):
 ...
 if flag_St == 0:
  # Actual_position = data.pose[0]
  # Actual_Speed = data.twist[0]
  Actual_Speed = data.data
  flag_St = 1

sub1 = rospy.Subscriber('/Longitudinal_Driving_Velocity', Float64, callback_Act_St)
...
def Pure_Pursuit_Control(Lane_Des,Pos_Act,V_Act):
  K_p = 0.5
  global Wheel_Base
  # print("Pos_Act[0]", Pos_Act[0], "np.sign(V_Act)", np.sign(V_Act))
